...
class EntropyModel(nn.Module):
    ...
    def __init__(
        self, likelihood_bound=1e-9, entropy_coder=None, entropy_coder_precision=16 # 8 / 16
        #  (in the default case, 16 bits, so all symbol probabilities must add up to 65536, which corresponds to 1
    ):
        super().__init__()
    ...
    def quantized_cdf(self):
        return self._quantized_cdf # (16, 27)
    ...
    def _quantize_fixed_point(self, param, bit_length=8, bit_length_integer=0, **unused):
        """
    ...
    def quantize(
        ...
        if mode == "noise":
            if torch.jit.is_scripting():
                half = float(0.5)
                noise = torch.empty_like(inputs).uniform_(-half, half)  # (C, 1, DHWB)
            else:
                noise = self._get_noise_cached(inputs)
            # print('quantize noise', noise.shape)
            inputs = inputs + noise
            return inputs
        ...
        # preserve the gradient, use the following: 
        with torch.no_grad():
        ...
        # outputs = self._quantize_fixed_point(outputs)['cluster_centers_']
    ...
    def compress(self, inputs, indexes, means=None):
        ...
        Args:
            inputs (torch.Tensor): input tensors (B, M, D, H, W)
            indexes (torch.IntTensor): tensors CDF indexes (B, M, D, H, W)
            means (torch.Tensor, optional): optional tensor means
        """
        symbols = self.quantize(inputs, "symbols", means)

        if len(inputs.size()) != 5: # 4D --> 5D
            raise ValueError("Invalid `inputs` size. Expected a 5-D tensor.")
    ...
    def decompress(self, strings, indexes, means=None):
        ...
        if len(indexes.size()) != 5:
            raise ValueError("Invalid `indexes` size. Expected a 5-D tensor.")
...
class EntropyBottleneck(EntropyModel):
    ...
    *tensorflow/compression*. See the original paper and the `tensorflow
    ...
    def forward(self, x):
        ...
        outputs = outputs.permute(4, 0, 1, 2, 3).contiguous()

        likelihood = likelihood.reshape(shape)
        likelihood = likelihood.permute(4, 0, 1, 2, 3).contiguous()
    ...
    def _build_indexes(size):
        N, C, D, H, W = size
        indexes = torch.arange(C).view(1, -1, 1, 1, 1)
        indexes = indexes.int()
        return indexes.repeat(N, 1, D, H, W) #default index, current channel index

    def compress(self, x):
        # x: (B, M, D, H, W)
        indexes = self._build_indexes(x.size())
        spatial_dims = len(x.size()) - 2
        medians = self._medians().detach().reshape(-1, *([1] * spatial_dims))
        medians = medians.expand(x.size(0), *([-1] * (spatial_dims + 1)))
        return super().compress(x, indexes, medians)
    
    def torchac_compress(self, x):
        # indexes = self._build_indexes(x.size())
        # spatial_dims = len(x.size()) - 2
        # medians = self._medians().detach().reshape(-1, *([1] * spatial_dims))
        # medians = medians.expand(x.size(0), *([-1] * (spatial_dims + 1)))

        # symbols = self.quantize(x, "symbols", medians)
        # print('x', x.shape, torch.max(x), torch.min(x)) 
        b = torch.squeeze(self.quantiles[:,:,0])
        symbols = self.quantize(x, "symbols", b[None, :, None,None,None])
        # print('symbols', symbols.shape, torch.max(symbols), torch.min(symbols), torch.min(b))

        if len(x.size()) != 5: # 4D --> 5D
            raise ValueError("Invalid `inputs` size. Expected a 5-D tensor.")
        if x.size() != indexes.size():
            raise ValueError("`inputs` and `indexes` should have the same size.")
        ...
        cdf = self._quantized_cdf.detach().cpu()
        cdf = cdf / cdf.max()
        cdf = np.repeat(cdf[np.newaxis, :, :], symbols.shape[0], axis=0)
        cdf = np.repeat(cdf[:, :,np.newaxis,:], symbols.shape[2], axis=2)
        cdf = np.repeat(cdf[:,:, :,np.newaxis,:], symbols.shape[3], axis=3)
        cdf = np.repeat(cdf[:,:, :, :, np.newaxis,:], symbols.shape[4], axis=4)
        
        # strings = []
        symbols = symbols.to(torch.int16).detach().cpu()
        # output_cdf = ...  # Get CDF from the model, shape B, C, d, h, w, Lp
        strings = torchac.encode_float_cdf(cdf, symbols, check_input_bounds=True, needs_normalization=True)
        # values = torchac.decode_float_cdf(cdf, strings, needs_normalization=True)
        # print('decode symbols', values[0], values.shape)
        # outputs = self.dequantize(values, b[None, :, None,None,None])
        # assert(torch.all(symbols.eq(values)))
        return strings, symbols
    
    def decompress(self, strings, size):
        output_size = (len(strings), self._quantized_cdf.size(0), *size)
        indexes = self._build_indexes(output_size).to(self._quantized_cdf.device)
        medians = self._medians().detach().reshape(-1, *([1] * len(size)))
        medians = medians.expand(len(strings), *([-1] * (len(size) + 1)))
        # medians = self._medians().detach().expand(len(strings), -1, 1, 1, 1)
        return super().decompress(strings, indexes, medians)
    
    def torchac_decompress(self, strings, size):
        # output_size = (len(strings), self._quantized_cdf.size(0), *size)
        # indexes = self._build_indexes(output_size).to(self._quantized_cdf.device)
        # medians = self._medians().detach().reshape(-1, *([1] * len(size)))
        # medians = medians.expand(len(strings), *([-1] * (len(size) + 1)))
        ...
        cdf = self._quantized_cdf.detach().cpu()
        cdf = cdf / cdf.max()
        cdf = np.repeat(cdf[np.newaxis, :, :], shape[0], axis=0)
        cdf = np.repeat(cdf[:, :,np.newaxis,:], shape[2], axis=2)
        cdf = np.repeat(cdf[:,:, :,np.newaxis,:], shape[3], axis=3)
        cdf = np.repeat(cdf[:,:, :, :, np.newaxis,:], shape[4], axis=4)
        
        values = torchac.decode_float_cdf(cdf, strings, needs_normalization=True)
        b = torch.squeeze(self.quantiles[:,:,0])
        outputs = self.dequantize(values, b[None, :, None,None,None])
        # print('dequantize', outputs.shape, torch.max(outputs), torch.min(outputs)) # torch.Size([256, 16, 2, 2, 2]) torch.Size([16, 25])
        return outputs
...
class GaussianConditional(EntropyModel):
    ...
    *tensorflow/compression*. See the `tensorflow documentation
    ...
    def _standardized_cumulative(self, inputs):
        ...
        return half * torch.erfc(const * inputs)  # cdf of gaussian #  erf(x): error function gives the probability of a random variable
    ...
    def _likelihood(self, inputs, scales, means=None):
        ...
        upper = self._standardized_cumulative((half - values) / scales) # standardize
    ...
    def build_indexes(self, scales):
        ...
        return indexes # dictionary index for reconstruction?s
